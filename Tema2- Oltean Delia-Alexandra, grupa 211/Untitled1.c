...
Persoana :: Persoana():IdPers(contor++)
{
    id = 0;
    nume = NULL;
}

Persoana :: Persoana(int id, char* nume):IdPers(contor++)
{
    this->id = id;
    this->nume=new char[strlen(nume)+1];
    strcpy( this->nume, nume);
...
Persoana :: Persoana(const Persoana& pers):IdPers(contor)
{
    this->id = pers.id;
